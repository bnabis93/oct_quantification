...
from pre_processing import shadow_augmentaiton
...
def temp_get_data_training(train_imgs_original,train_groudTruth,
    ...
    patches_imgs_train, patches_masks_train,class_freq_tabel = extract_oversampling(train_imgs,train_masks,patch_height,patch_width,num_subimgs,label_mapping,inside_FOV)
    ...
    visualize(group_images(patches_imgs_train[0:50,:,:,:],5),'./'+save_path+'/train_patch_img')
    visualize(group_images(patches_masks_train[::50,:,:,:] * 255,5),'./'+save_path+'/imgs_after_labels')
...
def ch_wise_one_hot_encoding(labels,class_num, mapping):
    ...
    return encoded_labels,class0_pixels,class2_pixels ,class2_pixels
    
#####################################################################################################################

#####################################################################################################################

def extract_oversampling(full_imgs,full_masks, patch_h,patch_w, num_patches,mapping, inside=True, ratio =1):
    ...
    sometimes2 = lambda aug: iaa.Sometimes(0.5, aug)
    
    seq = iaa.Sequential([
        sometimes2(iaa.Affine(scale=(0.7,1.3))),
        sometimes2(iaa.Affine(translate_percent={"x" : (-0.2,0.2), "y": (-0.2,0.2)})),
        sometimes2(iaa.Affine(shear=(-15,15))),
        sometimes2(iaa.Rotate((-30,30))),
        sometimes(iaa.MotionBlur(k=7, angle=[0,360])),
        sometimes(iaa.AdditiveGaussianNoise(scale=(0, 0.25*255))),
        #sometimes(iaa.MultiplyAndAddToBrightness(mul=(0.4, 1.7), add=(-40, 40))),
        sometimes(iaa.GammaContrast((0.65, 1.35)))
    ...
    iaa.Sometimes(
        0.5,
        iaa.Affine(shear= (-3,3), rotate=(-20,20),translate_percent={"x": (-0.1, 0.1), "y": (-0.1, 0.1)})
    )])'''
    ...
    for i in tqdm(range(full_imgs.shape[0]), desc = 'extract patches', mininterval = 10):  # loop, number of image samples
        ...
        bmo_row,bmo_col = np.where(np.all(temp_thr_label == (255,0,0), axis = -1))
        lc_row,lc_col = np.where(np.all(temp_thr_label == (0,0,255), axis = -1))
        bmo_len = len(bmo_row)
        lc_len = len(lc_row)
        
        
        aug_thr = int(patch_per_img * 0.7)
        while k<patch_per_img: #iter, number of patch per img
            ...
            random_idx = random.randint(0, temp_len-1)
            
            #x_center = random.randint(0+int(patch_w/2) , img_w-int(patch_w/2))
            #y_center = random.randint(0+int(patch_w/2) , img_h-int(patch_w/2))
            
            x_center = random.randint(temp_col[random_idx] - int(patch_w/3) , temp_col[random_idx] + int(patch_w/3))
            y_center = random.randint(temp_row[random_idx] - int(patch_w/3) , temp_row[random_idx] + int(patch_w/3))
            
            if x_center + patch_h > img_w:
                diff = (x_center+patch_h) - img_w
                x_center = x_center - diff 
                
            elif x_center - patch_h < 0:
                diff = - (x_center-patch_h)
                x_center = x_center + diff
            
            elif y_center + patch_h > img_h:
                diff = (y_center +patch_h) - img_h
                y_center = y_center -diff
            
            elif y_center - patch_h < 0:
                diff = - (y_center-patch_h)
                y_center = y_center +diff
            ...
            if choice_num == 2:
                patch = shadow_augmentaiton(patch[0])
                patch = np.expand_dims(patch, 0)
            
            #print('test patch shape : ', np.shape(patch))
            patch_mask,c0,c1,c2 = ch_wise_one_hot_encoding(full_masks[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)],num_class,mapping)
            
            
         

            if aug_thr < k :
                # 여기도 random shadow 가능
                
                # do augmentation
                aug_patch = np.transpose(patch,(1,2,0) )
                aug_patch_mask = np.transpose(full_masks[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)],(1,2,0))
                patch, patch_mask = seq(images = [aug_patch.astype(np.uint8)],
                                       segmentation_maps =[aug_patch_mask.astype(np.uint8)])
                
                patch = patch[0]; patch_mask = patch_mask[0]
                if temppp_cnt == 0 :
                    print('aug patch shape : {} aug mask shape : {}'.format(np.shape(patch),
                                                                            np.shape(patch_mask)))
                patch = np.transpose(patch, (2,0,1))
                patch_mask = np.transpose(patch_mask, (2,0,1))
                patch_mask,c0,c1,c2 = ch_wise_one_hot_encoding(patch_mask,num_class,mapping)
                temppp_cnt = temppp_cnt +1 
        ...
        class_frequency_table.loc[i,'frequency_0' ] = (class_frequency_table.loc[i,'class_0']) / (class_frequency_table.loc[i,'class_0'] +class_frequency_table.loc[i,'class_1'] + class_frequency_table.loc[i,'class_2'])
        
        class_frequency_table.loc[i,'frequency_1' ] = (class_frequency_table.loc[i,'class_1']) / (class_frequency_table.loc[i,'class_0'] +class_frequency_table.loc[i,'class_1'] + class_frequency_table.loc[i,'class_2'])
        
        class_frequency_table.loc[i,'frequency_2' ] = (class_frequency_table.loc[i,'class_2']) / (class_frequency_table.loc[i,'class_0'] +class_frequency_table.loc[i,'class_1']+ class_frequency_table.loc[i,'class_2'])
